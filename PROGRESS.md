...
| P4-C11 | Phase-4 verification + benches + adversarial review | ✅ done (10 findings fixed) |
...
- **Residuals (advisor-flagged at C9; status after the C11 sweep):** (1) mid-turn **steers are still dropped** in an orchestrated turn — `promote_pending_steers` runs only in the single-model loop; queued *prompts* still run as separate activities. STILL A CEILING (deliberate — folding steers into a fan-out is out of scope). (2) The **synthesizer sees no conversation history** — only `user_request` + candidate texts (candidates do get full history). STILL A CEILING (follow-up-context limitation). (3) `role_order` as a strict subset of `roles` — **FIXED at C11**: `validate()` now requires `role_order` (when set) to list every role (`RoleOrderIncomplete`). (4) abort-during-orchestration — **FIXED at C11**: added the live `abort_during_fan_out_settles_and_persists_nothing` coordinator E2E.
...
- **L2 — `role_order` subset silently dropped roles.** FIX: `validate()` now requires a non-empty `role_order` to list every role (`RoleOrderIncomplete`). Test `role_order_must_list_every_role`.
- **M1+M2 — watcher stored absolute paths / created duplicate rows.** On macOS, FSEvents delivers `/private/var/...` while the workspace root is the `/var/...` symlink, so `strip_prefix(root)` failed and the absolute fallback was stored (and the cold-start relative-keyed row was never replaced → duplicate). FIX: `reindex_path` canonicalizes the root and strips canonical-then-raw. Latent (no production watcher caller yet), but correct now on both macOS and Linux.
- **M3 — directory rename/delete orphaned child symbols.** A dir event delivers only the directory path (no per-file events); the old exact-match `remove_file` purged nothing. FIX: new `symbols::remove_under` (exact OR `dir/%` prefix, escaped) purges the subtree on a gone path, and `reindex_path` walks a created/renamed-IN directory to index its files. Tests: `remove_under_purges_directory_subtree_but_spares_siblings`, two watcher dir tests.
- **M4 — checkpoint timeline showed the wrong session.** No `clearCheckpoints()` on switch + no in-flight guard in `loadCheckpoints` (unlike `loadMessages`). FIX: `setActiveSession` now clears checkpoints, and `loadCheckpoints` gates its store write on `getDisplayedSession()`. Test `ignores a late load whose session is no longer displayed`.
...
| 5.12 | CLI packaging (`release.yml`, `install.sh`) | ✅ wired — `release.yml` builds the CLI on a `v*` tag across 5 targets (macOS x64/arm64, Linux x64/arm64, Windows x64) with per-artifact SHA256; `install.sh` resolves the matching triple. **Verified locally only for the HOST target (aarch64-apple-darwin):** `cargo build --release --locked -p private-code-cli` + `--help`. The other 4 targets are UNVERIFIED here — notably `aarch64-unknown-linux-gnu` is *cross-compiled* on x86_64 ubuntu and links C deps (libgit2-sys, openssl/keyring), the classic first-CI-run soft spot; expect possible iteration on the first remote run (the documented human "first green CI" step). Replaced the conflicting unused `dist-workspace.toml`. |
...
- **§3 Packaging — WIRED (human ceilings surfaced):** multi-target CLI release matrix + matching `install.sh` (CLI release build verified locally **for the host target only**; the other 4 — esp. the cross-compiled aarch64-linux — are unverified until the first remote run); desktop `tauri-action` job + valid `bundle` config — the desktop job is `continue-on-error`, so a green `release.yml` does NOT imply desktop bundles were produced. Human-only: code signing/notarization (certs), `cargo tauri icon` for macOS/Windows icon sets, first observed green remote CI (`gh` unauth here), package-manager formulas (need a shipped release to hash), and the Tauri in-app updater (signing key). Removed the unused conflicting `dist-workspace.toml`.
