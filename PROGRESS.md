...
- **C11 follow-up:** desktop dev-deps already carry providers/testkit + tempfile; the command layer has NO tests yet — that is exactly C11 (webview-free command harness driving a coordinator over an in-memory DB + ScriptedProvider).
- **C13 follow-up (unchanged):** wire the `delivery` arg (now accepted by `send_prompt`, defaults "steer"), the `set_model` deferred-bool, and deny-`feedback` into the frontend.
